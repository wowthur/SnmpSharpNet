...
namespace SnmpSharpNet
{
    ...
    public class SnmpV3Packet : SnmpPacket
    {
        ...
        public UserSecurityModel GetUSM(byte[] berBuffer, int length)
        {
            ...
            if (_protocolVersion != (int)ESnmpVersion.Ver3)
            ...
            if (_msgFlags.Authentication == false && _msgFlags.Privacy == true)
                throw new SnmpException(SnmpException.EErrorCode.UnsupportedNoAuthPriv,
            ...
            if (_securityModel.Value != _userSecurityModel.Type)
                throw new SnmpException(SnmpException.EErrorCode.UnsupportedSecurityModel,
        ...
        public byte[] Encode(byte[] authKey, byte[] privKey)
        {
            ...
            if (_msgFlags.Privacy && _userSecurityModel.EngineId.Length > 0)
            {
                ...
                byte[] encryptedBuffer = privacyProtocol.Encrypt(unencryptedPdu, 0, unencryptedPdu.Length, privKey, _userSecurityModel.EngineBoots, _userSecurityModel.EngineTime, out byte[] privacyParameters, auth);
        ...
        public byte[] GenerateAuthenticationKey()
        {
            ...
            {
                IAuthenticationDigest authProto = Security.Authentication.GetInstance(_userSecurityModel.Authentication);
                if (authProto != null)
                {
        ...
        public byte[] GeneratePrivacyKey()
        {
            ...
            if (authProto != null)
            {
                IPrivacyProtocol privProto = PrivacyProtocol.GetInstance(_userSecurityModel.Privacy);
                if (privProto != null)
                {
