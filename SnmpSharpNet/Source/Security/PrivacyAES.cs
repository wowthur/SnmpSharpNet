...
namespace SnmpSharpNet.Security
{
    ...
    {
        /// <summary>Salt value</summary>
        private long salt = 0;

        /// <summary>AES protocol key bytes. Valid values are 16 (for AES128), 24 (AES192) or 32 (AES256).</summary>
        private int keyBytes = 16; // Default is 128bit AES protocol
        ...
        public PrivacyAES(int keyBytes)
        {
            ...
            salt = Convert.ToInt64(rand.Next(1, int.MaxValue));
        }

        /// <summary>Get next salt Int64 value. Used internally to encrypt data.</summary>
        /// <returns>Random Int64 value</returns>
        protected long NextSalt()
        {
            if (salt == long.MaxValue)
                salt = 1;
            else
                salt += 1;
        ...
        /// <summary>Decrypt <see cref="ScopedPdu"/> BER encoded byte array.</summary>
        ...
        public byte[] Decrypt(byte[] cryptedData, int offset, int length, byte[] key, int engineBoots, int engineTime, byte[] privacyParameters)
        {
            ...
            decryptedData = decryptor.TransformFinalBlock(cryptedData, offset, length);

            return decryptedData;
        }

        /// <summary>Get minimum encryption/decryption key length required by the protocol.</summary>
        ...
        /// <summary>Calculates and returns length of the buffer that is the result of the encryption method.</summary>
        ...
        /// <summary>Privacy protocol name. Returns string "AES"</summary>
        ...
        public byte[] PasswordToKey(byte[] secret, byte[] engineId, IAuthenticationDigest authProtocol)
        {
            ...
            if (key.Length < MinimumKeyLength)
                return ExtendShortKey(key, secret, engineId, authProtocol);
