    ...
    public class Privacy3DES : IPrivacyProtocol
    {
        ...
        public byte[] Decrypt(byte[] encryptedData, int offset, int length, byte[] key, int engineBoots, int engineTime, byte[] privacyParameters)
        {
            ...
            if (encryptedData == null || encryptedData.Length < 8)
                throw new ArgumentOutOfRangeException(nameof(encryptedData), "Encrypted data buffer is null or smaller then 8 bytes in length.");

            if (offset > encryptedData.Length || (offset + length) > encryptedData.Length)
                throw new ArgumentOutOfRangeException(nameof(offset), "Offset and length arguments point beyond the bounds of the encryptedData array.");

            if (key == null || key.Length < 32)
                throw new ArgumentOutOfRangeException(nameof(key), "Minimum acceptable length of the decryption key is 32 bytes.");
        ...
        public byte[] ExtendShortKey(byte[] shortKey, byte[] password, byte[] engineID, IAuthenticationDigest authProtocol)
        {
            ...
            while (length < MinimumKeyLength)
            {
                ...
                int copyBytes = Math.Min(MaximumKeyLength - length, authProtocol.DigestLength);
